@echo off
REM 네컷 포토부스 합성 이미지 생성 - Windows 빠른 시작 스크립트
...
REM 1. 필수 폴더 생성
echo [1/4] 필수 폴더 생성 중...
if not exist "data\raw\normal" mkdir data\raw\normal
if not exist "data\train\fourcut" mkdir data\train\fourcut
echo √ 폴더 생성 완료
echo.

REM 2. 소스 이미지 확인
echo [2/4] 소스 이미지 확인 중...
dir /b data\raw\normal\*.jpg data\raw\normal\*.png 2>nul | find /c /v "" > temp_count.txt
